...
def agent_inference(
    ...
    _agent_log(prefix, f"start prompt={initial_text_prompt!r} image={img_path}")
    _agent_log(prefix, f"round={generation_count + 1} request")
    ...
    while generated_text is not None:
        ...
        _agent_log(prefix, f"round={generation_count + 1} tool={tool_call['name']}")
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                _agent_log(
                    prefix,
                    (
                        "duplicate segment_phrase prompt; "
                        f"prompt={current_text_prompt!r} requesting retry"
                    ),
                )
                ...
                _agent_log(
                    prefix,
                    f"segment_phrase prompt={current_text_prompt!r} used_prompts={len(USED_TEXT_PROMPTS)}",
                )
                ...
                if num_masks == 0:
                    _agent_log(prefix, "segment_phrase returned 0 masks")
                    ...
                    _agent_log(prefix, f"segment_phrase returned {num_masks} masks")
            ...
            for i in range(num_masks):
                _agent_log(prefix, f"examine_each_mask progress={i + 1}/{num_masks}")
                ...
                if "Accept" in verdict:
                    ...
                    _agent_log(prefix, f"mask {i + 1}/{num_masks} accepted")
                    masks_to_keep.append(i)
                elif "Reject" in verdict:
                    assert not "Accept" in verdict
                    _agent_log(prefix, f"mask {i + 1}/{num_masks} rejected")
                else:
                    raise ValueError(
            ...
            if len(masks_to_keep) == 0:
                _agent_log(prefix, "examine_each_mask kept 0 masks")
                messages.append(
                    {
                        "role": "user",
                        "content": [
                ...
                _agent_log(
                    prefix,
                    f"examine_each_mask kept {len(masks_to_keep)}/{num_masks} masks",
                )
                messages.append(
                    {
                        "role": "user",
            ...
            _agent_log(prefix, f"select_masks_and_return final_masks={len(masks_to_keep)}")
            return messages, final_outputs, rendered_final_output

        elif tool_call["name"] == "report_no_mask":
            _agent_log(prefix, "report_no_mask")
    ...
    _agent_log(prefix, "agent execution ended unexpectedly without final tool result")
    ...
    _agent_log(prefix, f"saved error history to {error_save_path}")
    raise ValueError(
