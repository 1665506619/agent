...
MAX_CONSECUTIVE_NEAR_DUPLICATE_PROMPTS = 3
MAX_CONSECUTIVE_ZERO_MASK_ROUNDS = 4
TERMINATION_REASON_NEAR_DUPLICATE = "near_duplicate_prompt_threshold"
TERMINATION_REASON_ZERO_MASK = "consecutive_zero_mask_threshold"
...
def _is_near_duplicate_prompt(prompt_a, prompt_b):
    tokens_a = _prompt_token_set(prompt_a)
    tokens_b = _prompt_token_set(prompt_b)
...
def _count_recent_near_duplicate_prompts(current_prompt, previous_prompts):
    count = 0
    for previous_prompt in reversed(previous_prompts):
        if _is_near_duplicate_prompt(current_prompt, previous_prompt):
            count += 1
        else:
            break
    return count
...
def _build_no_mask_response(img_path, termination_reason=None):
    ...
    return final_outputs, rendered_final_output
...
def agent_inference(
    ...
    FAILED_SEGMENT_PROMPTS = []
    consecutive_zero_mask_rounds = 0
    ...
    while generated_text is not None:
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            near_duplicate_count = _count_recent_near_duplicate_prompts(
                current_text_prompt, FAILED_SEGMENT_PROMPTS
            )
            if near_duplicate_count >= MAX_CONSECUTIVE_NEAR_DUPLICATE_PROMPTS:
                print(
                    "❌ Repeated near-duplicate segment phrases exceeded threshold, "
                    "returning no-mask."
                )
                messages.append(
                    {
                ...
                final_outputs, rendered_final_output = _build_no_mask_response(
                    img_path,
                    termination_reason=TERMINATION_REASON_NEAR_DUPLICATE,
                )
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                if num_masks == 0:
                    ...
                    FAILED_SEGMENT_PROMPTS.append(current_text_prompt)
                    consecutive_zero_mask_rounds += 1
                    if (
                        consecutive_zero_mask_rounds
                        >= MAX_CONSECUTIVE_ZERO_MASK_ROUNDS
                    ):
                        print(
                            "❌ Consecutive zero-mask rounds exceeded threshold, "
                            "returning no-mask."
                        )
                        final_outputs, rendered_final_output = _build_no_mask_response(
                            img_path,
                            termination_reason=TERMINATION_REASON_ZERO_MASK,
                        )
                        return messages, final_outputs, rendered_final_output
            ...
            if len(masks_to_keep) == 0:
                FAILED_SEGMENT_PROMPTS.append(LATEST_SAM3_TEXT_PROMPT)
                messages.append(
                    {
                        "role": "user",
                        "content": [
            ...
            final_outputs, rendered_final_output = _build_no_mask_response(img_path)
            messages.append(
                {
                    "role": "assistant",
                    "content": [{"type": "text", "text": generated_text}],
