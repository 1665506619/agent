    ...
    for attempt in range(max_retries + 1):
        try:
            if request_interval_s > 0:
                ...
                sleep_time = request_interval_s - elapsed
                if sleep_time > 0:
                    print(f"⏳ Sleeping {sleep_time:.2f}s before next model request...")
                    time.sleep(sleep_time)

            print(f"🔍 Calling model {model}... attempt={attempt + 1}/{max_retries + 1}")
            ...
            print(
                f"⏳ Rate limited. Retrying after {backoff:.2f}s "
